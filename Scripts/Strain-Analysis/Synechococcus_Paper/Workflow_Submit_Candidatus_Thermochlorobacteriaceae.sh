...
indx_path=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Candidatus_Thermochlorobacteriaceae
length=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Candidatus_Thermochlorobacteriaceae.fna.fai
sample=${s}
...
mkdir ${out_dir}
mkdir ${out_dir}Candidatus_Thermochlorobacteriaceae/
genome=Candidatus_Thermochlorobacteriaceae
genome_ref=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Candidatus_Thermochlorobacteriaceae.fna

wflow=/fs/cbcb-scratch/hsmurali/Hot_Springs_Analysis/Scripts/Strain-Analysis/Synechococcus_Paper/Workflow.sh
/usr/bin/time -v bash ${wflow} ${indx_path} ${length} ${sample} ${reads_1} ${reads_2} ${out_dir}Candidatus_Thermochlorobacteriaceae/ ${genome} ${genome_ref}
