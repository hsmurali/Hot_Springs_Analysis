...
indx_path=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Aerophilum
length=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Aerophilum.fna.fai
sample=${s}
...
mkdir ${out_dir}
mkdir ${out_dir}Aerophilum/
genome=Aerophilum
genome_ref=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Aerophilum.fna

wflow=/fs/cbcb-scratch/hsmurali/Hot_Springs_Analysis/Scripts/Strain-Analysis/Synechococcus_Paper/Workflow.sh
/usr/bin/time -v bash ${wflow} ${indx_path} ${length} ${sample} ${reads_1} ${reads_2} ${out_dir}Aerophilum/ ${genome} ${genome_ref}
