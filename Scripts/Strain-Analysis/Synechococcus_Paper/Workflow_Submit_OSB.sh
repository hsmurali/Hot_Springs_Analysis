...
indx_path=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Synechococcus_OS-B
length=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Synechococcus_OS-B_genome.fasta.fai
sample=${s}
...
mkdir ${out_dir}OSB/
genome=OSB
genome_ref=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Synechococcus_OS-B_genome.fasta

wflow=/fs/cbcb-scratch/hsmurali/Hot_Springs_Analysis/Scripts/Strain-Analysis/Synechococcus_Paper/Workflow.sh
/usr/bin/time -v bash ${wflow} ${indx_path} ${length} ${sample} ${reads_1} ${reads_2} ${out_dir}OSB/ ${genome} ${genome_ref}
