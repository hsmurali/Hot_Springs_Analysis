...
indx_path=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Synechococcus_OS-A
length=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Synechococcus_OS-A_genome.fasta.fai
sample=${s}
...
mkdir ${out_dir}OSA/
genome=OSA
genome_ref=${WRKPTH}Data/YNP_Hot_Springs/Ref_Genomes_Not_Syn/Synechococcus_OS-A_genome.fasta

/usr/bin/time -v bash Workflow.sh ${indx_path} ${length} ${sample} ${reads_1} ${reads_2} ${out_dir}OSA/ ${genome} ${genome_ref}
