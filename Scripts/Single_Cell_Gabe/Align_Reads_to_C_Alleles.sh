...
#SBATCH -o C_Cells_Rel_Abundance.CIGAR_SEP.o%j # Name of Output File
#SBATCH -e C_Cells_Rel_Abundance.CIGAR_SEP.e%j # Name of Error File
...
idx_dir=/fs/cbcb-lab/mpop/hotspring_metagenome/single_cell_analysis_with_Gabe_Birzu/C_Allele_Relative_Abundance_Aug/
c_idx=${idx_dir}all_C_alleles.idx
a_idx=${idx_dir}A_allele.idx
b_idx=${idx_dir}Bp_Allele.idx
...
wrkdir=/fs/cbcb-lab/mpop/hotspring_metagenome/single_cell_analysis_with_Gabe_Birzu/C_Allele_Relative_Abundance_Sep/

all_out_dir=${wrkdir}All_Alleles/
all_idx=${idx_dir}All_Alleles.idx
mkdir ${all_out_dir}
mkdir ${all_out_dir}bam_files/
mkdir ${all_out_dir}read_files/

bowtie2 --threads 8 -x ${all_idx} -U ${reads_dir}${sample}/QC_Filtered_Raw_Data/*.fastq.gz --no-unal | samtools view -bS > ${all_out_dir}bam_files/${sample}.bam
bedtools bamtofastq -i ${all_out_dir}bam_files/${sample}.bam -fq ${all_out_dir}read_files/${sample}.fq
gzip ${all_out_dir}read_files/${sample}.fq
...
bowtie2 --threads 8 -x ${c_idx} -U ${all_out_dir}read_files/${sample}.fq.gz --no-unal | samtools view -bS > ${c_out_dir}bam_files/${sample}.bam
...
bowtie2 --threads 8 -x ${a_idx} -U ${all_out_dir}read_files/${sample}.fq.gz --no-unal | samtools view -bS > ${a_out_dir}bam_files/${sample}.bam
...
bowtie2 --threads 8 -x ${b_idx} -U ${all_out_dir}read_files/${sample}.fq.gz --no-unal | samtools view -bS > ${b_out_dir}bam_files/${sample}.bam
